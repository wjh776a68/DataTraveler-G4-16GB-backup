...
void levelOrder(pTree &T,int v,int level,char *data)
{
	int w;
	*(data+v)=level;
	w=firstChild(T,v);
...
void TreeAmount(pTree &T)
{
	printf("树的节点总数为：%d\n",T.n);
}

void postOrder2(pTree &T,int v,int &count)
{
	int w;
	w=firstChild(T,v);
	if(w==-1)
	...
	while(w!=-1)
	{
		postOrder2(T,w,count);
		w=nextSibling(T,v,w);	
	}
}

void TreeLeaveCount(pTree &T)
{
	int i,count=0;
	int visited[MAXLEN];
	for(i=0;i<T.n;i++)
	...
	{
		if(T.node[i].parent==-1)
			postOrder2(T,i,count);
	}
	printf("森林叶子结点数为：%d",count);
}

void postOrder3(pTree &T,int v,int &degree)
{
	int w;
	int itdegree=0;
	w=firstChild(T,v);
...
void TreeDegree(pTree &T)
{
	...
	{
		if(T.node[i].parent==-1)
			postOrder3(T,i,degree);
	}
	printf("森林的度为：%d",degree);
}

void levelOrder2(pTree &T,int v,int level)
{
	int w;
	printf("(%c,%d)",T.node[v].data,level);
	w=firstChild(T,v);
	while(w!=-1)
	{
		levelOrder2(T,w,level+1);
		w=nextSibling(T,v,w);	
	}
}


void levelTraverse2(pTree &T)
{
	int i;
	int level=0;	
	int visited[MAXLEN];
...
//*******************************************************************//
int CreateTreeFromFile( pTree &T)
{
	char fileName[60];
